/*! HoloSplat v0.1.0 – WebGPU Gaussian Splat viewer | MIT */var HoloSplat=(function(){'use strict';const SHADER=`
...
overlay.style.display='none';const viewer=new Viewer({canvas,background,fov,near,far,splatScale,autoRotate,onProgress:p=>{bar.style.width=`${(p * 100).toFixed(0)}%`;if(onProgress)onProgress(p);},});async function load(url){showLoading();bar.style.width='0%';try{await viewer.load(url);showReady();if(onLoad)onLoad();}catch(err){const msg=navigator.gpu?err.message:'WebGPU not supported. Use Chrome 113+ or Edge 113+.';showError(msg);if(onError)onError(err);}}
viewer.init().then(()=>{viewer.start();if(src)return load(src);}).catch(err=>{const msg=navigator.gpu?err.message:'WebGPU not supported. Use Chrome 113+ or Edge 113+.';showError(msg);if(onError)onError(err);});return{load,destroy(){viewer.destroy();root.innerHTML='';root.classList.remove('hs-player');},setBackground(bg){viewer.setBackground(bg);},setSplatScale(s){viewer.setSplatScale(s);},setAutoRotate(v){viewer.setAutoRotate(v);},resetCamera(){viewer.resetCamera();},get camera(){return viewer.camera;},};}
function autoInit(){document.querySelectorAll('[data-holosplat]').forEach(el=>{if(el._hsPlayer)return;const src=el.getAttribute('data-holosplat');if(src)el._hsPlayer=player(el,{src});});}
...
return{create,player,Viewer,compressToSpz,encodeSpz,parseSplat,parsePly};})();